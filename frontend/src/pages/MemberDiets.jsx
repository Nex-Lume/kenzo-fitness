...
const MemberDiets = () => {
  ...
  const statusColors = {
    ...
    paused: 'text-[#c1ff00] bg-amber-500/10 border-amber-500/20',
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">View nutrition guidelines from your trainer.</p>
      </div>
            ...
            <div className="bg-emerald-500/10 border-b border-emerald-500/20 p-6 flex justify-between items-center">
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-md border ${statusColors[plan.status] || 'text-gray-400 bg-white/5'}`}>
                    {plan.status}
                  </span>
                  {plan.goal && (
                    <span className="text-[10px] font-bold uppercase px-2 py-1 rounded-md bg-zinc-950 text-gray-400 border border-white/10">
                      Goal: {plan.goal}
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {plan.meals?.map((meal, idx) => (
                    <div key={idx} className="bg-zinc-950 border border-white/10 p-5 rounded-xl">
                      ...
                      {meal.notes && <p className="text-xs text-gray-500 mt-2 italic">{meal.notes}</p>}
                    </div>
                  ))}
                  {(!plan.meals || plan.meals.length === 0) && (
                    <p className="text-sm text-gray-500">No specific meals defined.</p>
                  )}
                </div>

                {/* Macros & Water */}
                <div className="space-y-6">
                  <div className="bg-[#c1ff00]/5 border border-orange-500/20 p-6 rounded-xl">
                    <h3 className="text-base font-bold text-[#c1ff00] flex items-center gap-2 mb-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-gray-400 font-semibold uppercase">Protein</span>
                      ...
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-gray-400 font-semibold uppercase">Carbs</span>
                      ...
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-gray-400 font-semibold uppercase">Fats</span>
                          <span className="text-white font-bold">{plan.fat || 0}g</span>
                        </div>
                        <div className="h-2 bg-zinc-950 rounded-full overflow-hidden">
                  ...
                  <div className="bg-blue-500/5 border border-blue-500/20 p-6 rounded-xl flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-blue-400">{plan.water || 'Not specified'}</p>
                    </div>
                  </div>
                      ...
                      {plan.supplements && (
                        <div>
                          <h4 className="text-xs font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-2 mb-1">
                            <Info className="w-4 h-4" /> Supplements
                          </h4>
                          <p className="text-sm text-zinc-300">{plan.supplements}</p>
                        </div>
                      )}
                      {plan.notes && (
                        <div>
                          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">General Notes</h4>
